/**
 * x402-books — agent client example.
 *
...
async function main(): Promise<void> {
  ...
  const unpaid = await fetch(`${BASE_URL}/search?q=frankenstein&limit=2`);
  ...
  {
    const res = await payFetch(`${BASE_URL}/search?q=frankenstein&limit=2`);
    if (!res.ok) {
  ...
  {
    const res = await payFetch(`${BASE_URL}/read/84`);
    if (!res.ok) {
      console.error(`GET /read/:gutenbergId failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /read/:gutenbergId ($0.01) ===");
    console.log(JSON.stringify(await res.json(), null, 2));
